...
export function EnergyRing({
  ...
  useFrame((state) => {
    if (mesh.current) {
      ...
      // mesh.current.material can be a single material or an array; narrow and cast to MeshBasicMaterial
      const mat = Array.isArray(mesh.current.material)
        ? mesh.current.material[0]
        : mesh.current.material
      const basic = mat as THREE.MeshBasicMaterial
      basic.transparent = true
      basic.opacity = 0.5 + Math.sin(state.clock.elapsedTime * 3) * 0.3
    }
  })
